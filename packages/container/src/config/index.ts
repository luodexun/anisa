...
export class Config {
  ...
  public async setUp(opts): Promise<Config> {
    const network: Interfaces.INetworkConfig = this.configureNetwork(
      opts.network
    );
  ...
  ): Interfaces.INetworkConfig {
    const config: Interfaces.INetworkConfig = Managers.NetworkManager.findByName(
      network
    );
