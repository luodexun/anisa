...
export class Container implements container.IContainer {
  ...
  public resolveOptions(key) {
    return this.container.resolve<container.IPluginConfig<any>>(
      `pkg.${key}.opts`
    );
  }
  ...
  private registerExitHandler(exitEvents: string[]): void {
    const handleExit = async () => {
      ...
      if (this.isReady) {
        const logger: Logger.ILogger = this.resolvePlugin<Logger.ILogger>(
          "logger"
        );
        if (logger) {
          logger.suppressConsoleOutput(this.silentShutdown);
          logger.info(
            "Core is trying to gracefully shut down to avoid data corruption"
          );
        }
